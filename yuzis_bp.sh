#!/bin/bash
#Before you begin:
#1- Request for root access - prefered root user
#2- Make sure all nodes time synched
#3- List all master nodes before executing the script
...
if [[ $nfs_required = true ]]; then
  ...
  my_print "\nTASK [Configure nfs server] ****************************************************\n"
  {
    ...
    ssh $ssh_user@$nfs_server -p $ssh_port "echo $ssh_pass | sudo -S -p '' exportfs -a" &>> $log_file
  } &
  if [[ $? -eq 0 ]]; then
    my_print "$nfs_server" "changed"
  else
  ...
  for x in $(cat masters.txt); do
    {
      ...
      #ssh $ssh_user@$x -p $ssh_port "echo $ssh_pass | sudo -S -p '' exportfs -a"
      ssh $ssh_user@$x -p $ssh_port "echo $ssh_pass | sudo -S -p '' mount -a" &>> $log_file
    } &
    if [[ $? -eq 0 ]]; then
      my_print "$x" "changed"
    else
...
#my_print "\nTASK [Download installation file] ***********************************************\n"
#ggID='15Cd__uyISr12JtoXC3XRq4GRe-0KA1rZ'
...
bg_list=""
for x in $(cat masters.txt); do
  #if [ $(ssh -q $ssh_user@$x -p $ssh_port stat $filename) \> /dev/null 2\>\&1 ]; then

  if ssh $ssh_user@$x -p $ssh_port -q "test -e $filename"; then
    my_print "$x" "ok"
  else
     my_print "DO NOT COPY"
...
my_print "\nTASK [Load installation file] ***************************************************\n"
bg_list=""
for x in $(cat masters.txt); do
  #ssh $ssh_user@$x -p $ssh_port "echo $ssh_pass | sudo -S -p '' tar xf $filename -O | docker load" &>> $log_file &
  #ssh $ssh_user@$x -p $ssh_port "tar xf $filename -O | { echo $ssh_pass; cat -; } | sudo -S -p '' docker load" &>> $log_file &
  my_print "DO NOT LOAD DOCKER"
...
for x in $(cat masters.txt); do
  ...
  ssh $ssh_user@$x -p $ssh_port "docker rm $(docker ps -a -q)"  &>> $log_file
  ssh $ssh_user@$x -p $ssh_port "docker rmi $(docker images -q)" &>> $log_file
done
echo $ssh_pass | sudo -S -p '' rm -rf cluster/cfc* cluster/misc
